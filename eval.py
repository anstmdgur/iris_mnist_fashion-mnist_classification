...
def plot_confusion_matrix_and_report(model,dataloader,device,classes,config_name):
    ...
    print(classification_report(all_labels,all_predictions,target_names=classes,digits=4)) #presision, recall, f1 score 출력을 위한 함수
    #presision정밀도 = TP/TP+FP = FP(X가 아닌데 X라고 예측)이 높을수록 presision이 낮아진다. presision이 낮다면 헷갈릴때 그냥 X라고 예측하는 경우가 많음
        #presision이 높다면 모델이 x에 대해서 확실할때만 정답이라고 하는 경향이 있음
    ...
    #f1 score = presision과 recall의 조화평균. 둘 중 하나라도 값이 낮다면 f1 score가 확 깎인다. 데이터 클래스의 빈도 불균형이 심할때 훨씬 신뢰할 수 있는 지표.
